...
START_TEST(replaces_whitespaces_with_replacement) {
  char* str = slug_ascii("foo bar baz");
...
START_TEST(remove_not_allowed_chars) {
  char* str_1 = slug_ascii("foo, bar baz");
  ck_assert_str_eq(str_1, "foo-bar-baz");
  free(str_1);

  char* str_2 = slug_ascii("foo- bar baz");
  ck_assert_str_eq(str_2, "foo-bar-baz");
  free(str_2);

  char* str_3 = slug_ascii("foo] bar baz");
  ck_assert_str_eq(str_3, "foo-bar-baz");
  free(str_3);

  char* str_4 = slug_ascii("foo  bar--baz");
...
START_TEST(result_should_be_empty) {
  char* str_1 = slug_ascii("");
  ck_assert_str_eq(str_1, "");
  free(str_1);

  char* str_2 = slug_ascii(" ");
  ck_assert_str_eq(str_2, "");
  free(str_2);

  char* str_3 = slug_ascii("-");
  ck_assert_str_eq(str_3, "");
  free(str_3);

  char* str_4 = slug_ascii(" - ");
  ck_assert_str_eq(str_4, "");
  free(str_4);

  char* str_5 = slug_ascii("- -");
  ck_assert_str_eq(str_5, "");
  free(str_5);

  char* str_6 = slug_ascii("*");
  ck_assert_str_eq(str_6, "");
  free(str_6);

  char* str_7 = slug_ascii(" * ");
  ck_assert_str_eq(str_7, "");
  free(str_7);

  char* str_8 = slug_ascii("* *");
  ck_assert_str_eq(str_8, "");
  free(str_8);

  char* str_9 = slug_ascii("***");
  ck_assert_str_eq(str_9, "");
  free(str_9);

  char* str_10 = slug_ascii("-*-");
  ck_assert_str_eq(str_10, "");
  free(str_10);

  char* str_11 = slug_ascii("*-*");
